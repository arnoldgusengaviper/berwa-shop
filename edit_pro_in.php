...
<body>
    ...
    <form action="" method="post">
        <?php
        $id = $_GET['id'];
        $sele = "SELECT * FROM `product_in` WHERE `productcode` ='$id'";
        $result = mysqli_query($db,$sele);
        $ro = mysqli_fetch_array($result);
        ?>
        <h3>Update Product in</h3>
        ...
        if (isset($_POST['save'])) {
            ...
            $sql = "UPDATE `product_in` SET `productcode`='$pro_id', `date`='$date',`quantity`='$qua',`uniqueprice`='$u_price', `totalprice`='$t_price' WHERE `productcode` = '$id'";
